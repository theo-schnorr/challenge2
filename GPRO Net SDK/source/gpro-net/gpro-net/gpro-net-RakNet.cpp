...
namespace gproNet
{
	...
	void cRakNetManager::SendServerMessage(RakNet::BitStream& bitstream, RakNet::MessageID id, char const message[])
	{
		WriteTimestamp(bitstream);
		bitstream.Write((RakNet::MessageID)id);
		bitstream.Write(message);
		
		// Send  bitstream to the server, server figures out what to do with info
	}
	...
	int cRakNetManager::MessageLoop()
	{
		...
		while (packet = peer->Receive())
		{
			...
			// Take the message id, send it to the funky switch statement
